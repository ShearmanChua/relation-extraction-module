...
class spanREL(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        span_pairs_w_preds = torch.tensor([[*span_pair, pred.cpu().detach().item(), conf.cpu().detach().item()]
                                           for span_pair, pred, conf in zip(flattened_span_pairs, preds, confidence)], device=self.device)
        ...
        predictions_with_labels = [[(*span_pair[:2], *span_pair[3:5], relation_labels[int(span_pair[-2])], span_pair[-1]) for span_pair in sample.cpu().detach().tolist() if span_pair[-2] != 0]
        ...
        for idx,sample in enumerate(predictions_with_labels):
            ...
            for span in sample_spans:
                # print(span)
                # print(sample_tokens)
                subj_span_tokens = sample_tokens[int(span[0]):int(span[1])]
                obj_span_tokens = sample_tokens[int(span[2]):int(span[3])]
                relation_label = span[4]
                # print(subj_span_tokens, obj_span_tokens, relation_label)
                confidence = round(span[5],2)
                if confidence > self.args.rel_confidence:
                    try:
                        subj_span = self.tokenizer.convert_tokens_to_string(subj_span_tokens)
                        obj_span = self.tokenizer.convert_tokens_to_string(obj_span_tokens)
                        sub_indices_object = re.finditer(pattern=subj_span, string=texts[idx])
                        obj_indices_object = re.finditer(pattern=obj_span, string=texts[idx])
                        sub_indices = [index.span() for index in sub_indices_object]
                        obj_indices = [index.span() for index in obj_indices_object]
                        span_predictions = [(sub_indices, obj_indices, relation_label, subj_span, obj_span,confidence)]
                        predictions.extend(span_predictions)
                    except:
                        print("Unable to process tokens: ")
    ...
    def predict_step_end(self, outputs):
        ...
        for prediction_batch in outputs:
